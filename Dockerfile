...
# Додаємо pipx та deno у PATH перед встановленням yt-dlp
ENV PATH="/root/.local/bin:/root/.deno/bin:$PATH"

# Встановлення yt-dlp з залежностями за замовчуванням (включає yt-dlp-ejs)
# Deno буде використано автоматично як JavaScript runtime для розв'язання YouTube викликів
RUN pipx install "yt-dlp[default]" && \
    mkdir -p /root/.config/yt-dlp && \
    echo "remote-components ejs:github" > /root/.config/yt-dlp/config.txt && \
    deno --version && \
    yt-dlp --version
